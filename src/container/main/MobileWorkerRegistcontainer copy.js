...
import { generateAndSaveAIImageForWorker } from "../../service/AIImageService";
...
const TagItem = styled.div`
 padding: 6px 10px;
  ...
  background: ${({ active }) => (active ? "#ff7e19" : "#fff")};
  color: ${({ active }) => (active ? "#fff" : "#000")};
  border: ${({ active }) => (active ? "2px solid #ff7e19" : "1px solid #ccc")};
`;
...
const SelfIntroInputBlock = ({ value, onChange, error, inputRef, userMeta, setUsedAISelfIntro }) => {
  ...
  const handleAIRewriteClick = () => {
    if (!value || value.length < 30) {
      toast.error("AI 지원서는 자기소개 30자 이상 입력 시 생성할 수 있어요!");
      return;
    }
    setShowPopup(true);
  };
  ...
  return (
    <div ref={inputRef} style={{ marginBottom: "24px" }}>
      ...
      <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
        간단한 자기소개를 입력하신 내용을 바탕으로 AI가 자연스럽게 이어서 작성해드립니다.
      </div>
      ...
      {showPopup && (
        <AIPopup
          ...
          onApply={(newText) => {
            console.log("✅ newText:", newText, typeof newText); // 이게 string이어야 함

            if (typeof newText === "string") {
              const trimmed = newText.startsWith("서") ? newText.slice(1) : newText;
              onChange({ target: { value: trimmed } }); // ✅ 부모로 전달
              toast.success("AI 지원서가 적용되었습니다!");
              setUsedAISelfIntro(true); // ✅ 요거 한 줄 추가!
              setShowPopup(false);
            } else {
              console.error("❌ newText가 문자열이 아님!", newText);
            }
          }}
  ...
  return (
    <div style={{ marginBottom: "24px", marginTop:"50px" }}>
      ...
      <InfoBox>
        ...
        선택하신 능력은 <strong style={{ color: "#ff7e19" }}>‘우리 동네 능력자 추천’</strong>에 노출됩니다.<br />
        <br />
        예시:<br />
...
const MobileWorkerRegistContainer = ({ editMode = false, existingData = {}, promptMode }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const profileData = {
        gender, profileImg, age: selectedAge, tags: selectedTags,
