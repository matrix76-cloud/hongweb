...
import EditToggleBlue from "../../components/EditToggleBlue";
...
const Container = styled.div`
  ...
  -webkit-overflow-scrolling: auto;                                    // ✅ 스크롤 자연스러움
  background-color: #fcfbf7;                                           // ✅ 톤 통일 (카드와 잘 어울림)
...
const SupporterLayer = styled.div`
  ...
  background: #FFF;
  margin: 20px auto;
  padding: 15px 16px 15px 9px;  // ✅ 왼쪽 padding 보정
...
const SupporterLayer2 = styled.div`
  position: relative;
  background: #FFF;
  margin: 20px auto;
  padding: 15px 16px 15px 9px;  // ✅ 왼쪽 padding 보정
...
// padding: 20px 16px 16px 16px; // 위쪽 padding 살짝 더 줘서 내용 밀리지 않게
...
const CategoryLabel = styled.div`
  ...
  padding: 6px 20px;           // 좌우 padding 증가
  border-radius: 9999px;       // pill 형태로 완전히 둥글게
...
`;
 ...
 const PetItemRow = styled.div`
  display: flex;
  justify-content: space-around;   // 간격 자동 분배
  align-items: center;
...
const WorkGradient = styled.div`
  ...
  background: linear-gradient(120deg, #4FC3F7, #81D4FA, #B3E5FC, #4FC3F7);
  background-size: 400% 400%;
  animation: borderSmooth 4s ease-in-out infinite;
  z-index: 0;
  pointer-events: none;

  @keyframes borderSmooth {
...
const TabButton = styled.div`
  ...
  color: ${({ active }) => (active ? COLORS.primary : "#131313")};
...
const WorkStatusAlert =() => {
  ...
  //     const result = await getJobList({ latitude, longitude });


  //     const fullAddress = user.USERINFO.address_name || "";
  //     const baseCity = extractCityName(fullAddress);  // ex: '서울시', '남양주시'

  //     const result2 = await getGeneralJobs({ addressName: baseCity });


  //     setWorkCount(result.length + result2.length || 0);

  //     setIsLoading(false); // 로딩 종료
  //   };

  //   // 위치정보가 있을 때만 fetch
  //   if (user?.USERINFO?.latitude && user?.USERINFO?.longitude) {
  //     fetchJobs();
  //   }
  // }, [user?.USERINFO?.latitude, user?.USERINFO?.longitude]);
  ...
  const newText = workCount > 0
    ? `${workCount}개 일자리가 등록되어 있습니다.`
    : "신규 일감 없음";
  const WorkView = () => {
...
const HongStatusAlert = ({ status }) => {
  const navigate = useNavigate();

  const { user } = useContext(UserContext);
  const newcount = user?.workerstatus?.newcount;
  ...
  const hongView = () => {
    navigate("/Mobilworkmap", { state: { WORK_ID: "", TYPE: "" } });
  }

  const newText = newcount === undefined
    ? "⏳ 회원님 정보를 불러오는 중이에요..."
    : newcount > 0
      ? `${newcount}명이 아르바이트를 위해 대기 중이에요.`
      : "아직 아르바이트 지원한 회원님이 없으십니다";

  
  // const newText = " ${newCount}명의 홍여사의 아르바이트를 하기위해 대기중에 있습니다";
...
const AlertGradient = styled.div`
  ...
  border-radius: 8px;
  border: 2px solid transparent;
  background: linear-gradient(120deg, #FFB74D, #FFE082, #FFF3C0, #FFB74D);
  background-size: 400% 400%;
  animation: borderSmoothReverse 4s ease-in-out infinite;
  z-index: 0;
  pointer-events: none;

  @keyframes borderSmoothReverse {
...
const AlertGradient2 = styled.div`
  ...
  border-radius: 8px;
  border: 2px solid transparent;
  background: linear-gradient(120deg, #d3d3d3, #eeeeee, #fafafa, #d3d3d3);
  background-size: 400% 400%;
  animation: borderSmoothReverse 4s ease-in-out infinite;
  z-index: 0;
  pointer-events: none;
...
const TaskPreviewBox = styled.div`
  display: grid;
...
const SectionBanner = styled.div`
  width: 100vw;
  margin-left: -16px;  // ✅ 기존 padding 제거
  margin-right: -16px;
  padding: 0 16px;
  background: ${({ bgcolor }) => bgcolor || "#F8F8F8"};
  padding-top: 24px;
  padding-bottom: 40px;  // ✅ 아래 박스와 확실히 분리
`;
...
const recommendedList = [
  {
    ...
    gender: "남성",
    tags: ["가이드영상", "영상등록안내"],
    videoUrl: "https://firebasestorage.googleapis.com/v0/b/help-bbcb5.appspot.com/o/introduce%2F%5B%E1%84%87%E1%85%A9%E1%86%A8%E1%84%89%E1%85%A1%E1%84%87%E1%85%A9%E1%86%AB%5D%20%E1%84%89%E1%85%B5%E1%86%AB%E1%84%82%E1%85%B2%E1%84%89%E1%85%B3%20(1).mp4?alt=media&token=d10a6dbc-9e50-40d3-ab3c-492aee501898", // ✅ 업로드된 서버 URL
    videoThumbnail: "", // 또는 유튜브 썸네일
...
const HorizontalScrollWrapper = styled.div`
  ...
  max-width: 100vw;       // 혹시라도 이상한 브라우저에서 넘치지 않게
  /* 스크롤바 감추기 */
...
// ★ NEW: 히어로 카드 (구인/구직 공통)
// 모바일 전용 히어로 카드 레이아웃 (텍스트 왼쪽 / 캐릭터 오른쪽, 겹침 없음)
const WorkBannerBox = styled.div`
  position: relative;
   display: grid;
  grid-template-columns: minmax(0,1fr) 10px; /* ← 128 → 120로 */
  ...
  /* ✅ 번짐(그림자)도 variant에 맞춰 변경 */
  box-shadow: ${({ variant }) =>
    variant === "seek"
      ? "0 16px 40px rgba(60,140,255,.33)"  // 파란 번짐
      : "0 16px 40px rgba(122,64,255,.35)"}; // 보라 번짐
  ...
  /* 컨테이너 ::after 접지는 제거 — 캐릭터 래퍼에서만 생성 */
`;
...
const HeroFigure = styled.div`
  grid-area: char;
  justify-self: end;
  align-self: start;   /* ← 위쪽 붙이기 */
  position: relative;
  width: 140px;        /* 텍스트 공간 더 확보 */
  margin-right: -6px;
  margin-top: 6px;

  /* 바닥 접지(캐릭터와 함께 따라 올라감) */
  &::after{
    content: "";
    position: absolute;
    left: 50%;
    ...
    background: radial-gradient(ellipse at center,
      rgba(0,0,0,.24) 0%, rgba(0,0,0,.12) 45%, rgba(0,0,0,0) 70%);
    filter: blur(6px);
    opacity: .35;
    pointer-events: none;
  }
...
const HeroTitle = styled.div`
  font-family: Pretendard-Bold, sans-serif;     /* ExtraBold → Bold */
  font-size: ${() => getFontSize(28)}px !important;  /* 38 → 32 */
  line-height: 1.05;
  letter-spacing: -0.5px;
  @media (max-width: 390px){ font-size: ${() => getFontSize(28)}px !important; }
`;

const HeroLine = styled.div`
  margin: 10px 0 16px;
  font-size: ${() => getFontSize(20)}px !important;
  line-height: 1.32;  /* ← 두 줄 가독성 */
  color: rgba(255,255,255,.92);
    word-break: keep-all;      /* 한글 단어 중간 줄바꿈 방지 */
`;

const Line = styled.span` display:block; `;
const NoWrap = styled.span` white-space: nowrap; `;  // ← 2번째 줄은 절대 안 꺾임
const EmNumber = styled.span`
  display: inline-block;
  background: linear-gradient(180deg, #FFC83A 0%, #FF7A00 100%);
  ...
  /* ✅ 줄 높이/정렬 흔들림 방지 */
  line-height: 1;
  transform: translateY(1px);     /* 또는 vertical-align: -0.06em; */
  font-variant-numeric: tabular-nums; /* 숫자 폭 안정 */
`;
...
const GlassButton = styled.button`
  ...
  color: #fff;
  font-size: ${() => getFontSize(16)}px !important;  /* 18 → 16 */
  font-family:Pretendard-SemiBold;
  cursor: pointer;
...
const WorkIllustration2 = styled.img`
  width: 110px;
  margin-bottom: 20px;
`;
...
const MobileMaincontainer = ({ containerStyle, checkingPermission, showUpdateBanner }) =>  {
  ...
  const [selectedTab, setSelectedTab] = useState("hire"); // seek | hire
  ...
  const { visibleKeys, replace } = useCategoryPrefs({ autoSave: false });
  ...
  console.log("MobileMaincontainer Worknet center/radius", center, radiusKm);

  // 선택된 카테고리 총합
  const selectedTotalCount = useMemo(
    () => visibleKeys.reduce((sum, key) => sum + (nearCounts?.[key] || 0), 0),
    [nearCounts, visibleKeys]
  );


  // syncFirst3UserImgs();

  //cleanWorkersWithoutPhone();


  // updateAllUserImagesIfMatched();

  // useCustomBackHandler(); // ✅ 뒤로가기 작전 투입

  const handleCategorySaved = (nextKeys) => {
    // 상태 반영 + 로컬스토리지 저장(한 줄)
  ...
  useEffect(() => {
    ...
    const fetchNewWorks = async () => {
      ...
      const result = await getNewWorksSinceLastLogin({ uid: user.USERS_ID, lastLoginAt: lastLogin, location: { latitude: user.USERINFO.latitude, longitude: user.USERINFO.longitude } });
      if (result.length > 0) {
        setNewWorks(result);
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      setIsCardTouchable(true);
    }, 500);
    return () => clearTimeout(timeout);
  }, []);

  // ✅ WebSimulator + 위치 없으면 mock 위치 주입
  // useEffect(() => {
  //   const isWebSimulator = !window.ReactNativeWebView;
  ...
  //   ) {
  //     setTimeout(() => {
  //       dispatch({ USERINFO: { latitude: 37.6106, longitude: 127.1626, address_name: "경기도 남양주시 다산동" } });
  //     }, 1000);
  //   }
  // }, [user.USERINFO?.latitude, user.USERINFO?.longitude]);

  // ✅ RN에서 권한 요청 트리거 (시뮬은 제외)
  useEffect(() => {
  ...
  // ✅ 권한 denied → denied_retry 상태 전환

  useEffect(() => {
    if (
      user.popupStep === "denied" &&
      isPermissionRetried &&
      (!user.locationGranted || !user.pushGranted)
    ) {
  ...
  //     const total = await getWorknetTotalCount();
  //     console.log("📦 worknet_v2_jobs 총 문서 수:", total.toLocaleString());


  //     const items = await ReadAllWorkTmp();
  ...
  //   console.log("useEffect 0615coupon");

  //   const checkCouponVisibility = async () => {
  //     const hiddenUntil = await safeGetCouponHiddenUntil();
  //     const isHidden = hiddenUntil && today <= hiddenUntil;
  ...
  //       !isHidden
  //     ) {
  //       setCouponPopupEvent(true); // ✅ 쿠폰 객체 대신 true
  //     }
  //   };
  ...
  const handleOneDay = async () => {
    ...
    setCouponPopupEvent(null); // 팝업 닫기
  };
  ...
  // 3개씩 보여주므로 그룹 인덱스는 hour % 총 그룹 수
  const groupSize = 3;
  const groupIndex = Math.floor(hour % Math.ceil(BannerItems.length / groupSize));
  const startIndex = groupIndex * groupSize;

  // const bannersToday = BannerItems.slice(startIndex, startIndex + groupSize);

  const bannersToday = BannerItems;


  const handleSelfMoreClick = () => {
    navigate("/MobileIntroduceList");
  }
  ...
  const _handleWorkerView = () => {
      navigate('/Mobileworkermap');
  }

  const _handleWorkRegister = () => {
  ...
  const onSeeAll = () => {
    navigate("/MobileResult", { state: { NAME: "구직자 전체보기", TYPE: "worker" } });
  }


    const _handleRegiserConfess = () => {
                  ...
                  selectedTab == "seek" ? (
                    <>
                      ...
                      <WorkBannerBox variant="seek">
                        <HeroTexts>
                          <HeroTitle>구직하기</HeroTitle>
                
                          <HeroLine>
                            <Line>총 <EmNumber>{user?.workerstatus?.newcount ?? 0}</EmNumber>명이 알바를 위해</Line>
                      ...
                      <BannerCard onClick={() => { navigate("/MobileAIView") }}>
                        ...
                        <TextGroup>
                          <Title>{'AI가 지원서 분석하여 그린 우리 동네 알바들'}</Title>
                      ...
                      <AbilityHorizontalSlider workeritems={workeritems} />

                      {
                        user.workeritems.length > 0 && <Section>
                      ...
                      {/* <FunctionSwipeSection width={width - 40} /> */}
                    </>
                  ) : (
                    ...
                    <>
                      <WorkBannerBox variant="hire" >
                        ...
                        <BannerCard onClick={_handleRegiserConfess}> {/* ✅ 등록 경로는 실제 등록 페이지에 맞춰 수정 */}
                          <img src={imageDB.ic_myinfo_menu_mypraise} style={{ width: 60, height: 60 }} />
                          <TextGroup>
                            <Title>우리 가게도 자랑해보세요!</Title>
                            <Subtitle>AI가 이미지와 문구, 음성까지 자동으로 만들어드려요</Subtitle>
                          </TextGroup>
                        </BannerCard>
                        ...
                        <Row style={{ justifyContent: "space-between", alignItems: "center", marginBottom: 12, marginTop: 40 }}>
                          ...
                          {open && (
                            <CategoryEditSheet
                              ...
                              items={DEFAULT_ITEMS}
                              countsMap={nearCounts}     // ✅ 전체 카테고리 건수 전달
                        ...
                        <JobCategoryGrid
                          items={DEFAULT_ITEMS}     // 모든 카테고리 정보
                          visibleKeys={visibleKeys} // ✅ 현재 사용자 설정(스토리지) 반영
                          countsMap={nearCounts}     // 선택(있으면 건수 표시)
                          isLoading={true}
                        /> 
