...
import { ReadFESTITVALBYFESTIVAL_ID } from '../service/FestivalService';
...
export default function MobileFestivalPopupView({containerStyle}) {
  const [open, setOpen] = React.useState(true);
  const [refresh, setRefresh] = React.useState(-1);

  const [searchParams] = useSearchParams();
  const [festival_id, setFestival_id] = React.useState(searchParams.get('id'));// URL 쿼리에서 id 가져오기
  const [item, setItem] = React.useState({});
  const [sanitizedHtml, setSanitizeHml] = React.useState({});
  const [sanitizedHtml2, setSanitizeHml2] = React.useState({});
  ...
  React.useEffect(()=>{
    setItem(item);
    setSanitizeHml(sanitizedHtml);
    setSanitizeHml2(sanitizedHtml2);
    setLoading(loading);
  },[refresh])
  ...
  React.useEffect(()=>{
    async function FetchData() {
      ...
      const FESITVAL_ID = festival_id;

      console.log("FESTIVAL_ID", FESITVAL_ID);
      const itemTmp = await ReadFESTITVALBYFESTIVAL_ID({ FESITVAL_ID })

      console.log("itemTmp", itemTmp);
      setSanitizeHml(DOMPurify.sanitize(itemTmp.FESTIVALITEM.fstvlCrmnCn));
      setSanitizeHml2(DOMPurify.sanitize(itemTmp.FESTIVALITEM.fstvlOutlCn));
      ...
      var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
        mapOption = {
          center: new kakao.maps.LatLng(itemTmp.FESTIVALITEM.xcrdVal, itemTmp.FESTIVALITEM.ycrdVal), // 지도의 중심좌표
          level: 5 // 지도의 확대 레벨
        };
      ...
      const markerPosition = new window.kakao.maps.LatLng(itemTmp.FESTIVALITEM.xcrdVal, itemTmp.FESTIVALITEM.ycrdVal);
      ...
      >
        <Fade in={open}>
          <Box sx={style}>
            ...
            {loading == false && <Container style={containerStyle}>
              ...
              <InstallButton href="https://honglady.co.kr" target="_blank">
                👇 앱 설치하고 축제 정보 더 보기 
              </InstallButton>
             </div>
