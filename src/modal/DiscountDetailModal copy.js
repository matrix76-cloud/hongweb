    ...
    return (
        <Overlay>
            <ModalBox onClick={(e) => e.stopPropagation()}>
                ...
                <BrandTag brandColor={getBrandColor(item.site)}>{getBrandLabel(item.site)}</BrandTag>
                <h3>{item.title}</h3>
                ...
                {item.originalPrice && (
                    <div style={{ textDecoration: 'line-through', fontSize: getFontSize(13), color: '#aaa' }}>
                ...
                {item.discountRate > 0 && (
                    <div style={{ fontSize: 14, color: '#FF5722', marginTop: 6 }}>
                        🔥 {item.discountRate}% 할인중!
                    </div>
                )}

                <LinkButton onClick={_handleOpenLink}>
                    상품 보러가기
                </LinkButton>
            </ModalBox>
        </Overlay>
