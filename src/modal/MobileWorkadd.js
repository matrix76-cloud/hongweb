...
export default function MobileWorkadd({date, callback}) {
  ...
  const _handleExecChage = (execday)=>{
    console.log("execday", execday);
    setStartdate(execday);
    setRefresh((refresh)=> refresh +1);
  }
  ...
  return (
    <div>
      ...
      >
        ...
        <input  type="text" value ={memo} className="custom-input"
        ...
        <Row style={{ width: "85%", margin: "15px auto" }}>
          ...
          <Label>담당자</Label>
          <input type="text" value={name}  className="custom-input"
            placeholder={'예)아들, 아빠, 큰딸'}
            maxLength={6} // 6자 제한
            onChange={(e) => {
...

        {
          alarmcheck == true  &&
            <Row style={{ width: "85%", margin: "5px auto" }}>
              <Label style={{width:'35%'}}>전화번호</Label>
              <input type="number" value={phone}  className="custom-input"
