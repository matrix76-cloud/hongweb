...
export default function MobileWorkMapPopup({filterhistory, callback}) {
  const [open, setOpen] = React.useState(true);
  const [refresh, setRefresh] = React.useState(-1);
  ...
  const handleComplete = async(data) => {
    console.log("TCL: handleComplete -> data", data)
    // 선택된 주소를 처리하는 로직
    ...
    console.log("TCL: handleComplete -> fullAddress", fullAddress)
      ...
      >
        <Fade in={open}>
          <Box sx={style}>
             ...
             <Row>
              <div style={{display: "flex",width: '70%',lineHeight: 1.8, margin:"20px 0px"}}>개인정보를 위해 대략적으로 표시해주세요. 상세한 주소는 채팅을 통해 전달해 주세요</div>
             </Row>

            <MainData>
              <DaumPostcode onComplete={handleComplete} style={{height:"350px"}} />
