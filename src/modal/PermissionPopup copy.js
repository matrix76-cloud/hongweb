...
const PermissionPopup = ({ popupStep, locationGranted, pushGranted, onClose, onConfirm, isPermissionRetried, setIsPermissionRetried }) => {
    ...
    <Overlay>
      <PopupBox>
        ...
        <div style={{ padding: "10px 15px", background: "#ededed" }}>
          ...
          <FooterText>
            권한을 허용하시면 홍여사의 모든 기능을 편리하게 사용할 수 있어요.
          </FooterText>
        </div>
        ...
        {popupStep === 'denied' && (
          <>
            <Description>
              위치 권한이 꺼져 있어요.<br />
              아래 버튼을 눌러 권한을 요청해 주세요.
            </Description>
            <ButtonRow>
              <ConfirmButton onClick={handleInitialPermissionRequest}>권한 요청하기</ConfirmButton>
            </ButtonRow>
          </>
        )}

        {popupStep === 'denied_retry' && (
          <>
            <Description>
              일부 권한이 아직 허용되지 않았어요.<br />
              필요한 항목을 설정하거나 다시 확인해 주세요.
            </Description>
            <ButtonRow>
