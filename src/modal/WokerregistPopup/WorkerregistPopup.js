...
import { CONFIGMOVE, FILTERITEMPERIOD } from '../../utility/screen';
...
export default function WorkerregistPopup({callback, content}) {
  const reduxdispatch = useDispatch();
  const [open, setOpen] = React.useState(true);
  const [refresh, setRefresh] = React.useState(-1);
  const navigate = useNavigate();

  const handleHongMove = () =>{    
  ...
  return (
    <div>
      ...
      >
        <Fade in={open}>
          <Box sx={style}>
              <Row style={{width:"90%", margin: "15px auto", height:'40px'}}>
                <IconCloseView >
             ...
             <Row style={{width:"100%", background:"#E3E3E3", height:'45px',borderBottomRightRadius: "10px",
              borderBottomLeftRadius: "10px"}}>
                <CheckText onClick={handleHongMove}>
                  일꾼으로 등록하러 가기
                </CheckText>
             </Row>
          </Box>
