...
const CommonHeaderWorker = ({ onLocationClick, onFilterClick, onBackPressed, filterSummary, onFilterReset }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(updateFilterCount, 5000); // 주기적으로 체크


    window.addEventListener('storage', updateFilterCount);
    return () => window.removeEventListener('storage', updateFilterCount);
  }, []);

  const renderFilterSummaryBar = () => {
    if (!parsedFilterCount || parsedFilterCount === 0) return null;
    return (
      <FilterSummaryBar onClick={onFilterClick}>
        <span>필터 {parsedFilterCount}개 적용됨</span>
      ...
      <HeaderWrapper>
        ...
        <RightArea>
          <FilterButton onClick={onFilterClick}>
            <FiFilter size={20} color="#333" />
            {parsedFilterCount > 0 && (
              <FilterBadgeBubble>{parsedFilterCount}</FilterBadgeBubble>
            )}
          </FilterButton>
        </RightArea>
      </HeaderWrapper>
...
export default CommonHeaderWorker;
