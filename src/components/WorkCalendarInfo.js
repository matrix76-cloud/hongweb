...
const WorkCalendarInfo = ({ containerStyle, workitems, selectstartorigin }) => {
  ...
  const _handleComplete = async(data) => {
  
    setSaving(true);
    const HOMEWORK_ID = data.HOMEWORK_ID;

    await UpdateHOMEWORKid({ HOMEWORK_ID });
    await sleep(1000);
    const USERS_ID = user.USERS_ID;
    const homeworkitems = await ReadHomeWork({ USERS_ID });
    setHomeworkmenu(homeworkitems);

    console.log("homeworkitemsTmp", homeworkitems, selectstartorigin);
    let homeworkitemsTmp = [];
    homeworkitems.map((data, index) => {
      if (moment(data.STARTDATE).isSame(moment(selectstartorigin))) {
    ...
    homeworkitemsTmp.map((data) => {
      if (data.complete == false) {
        uncompleteworkitemsTmp.push(data);
      } else {
        completeworkitemsTmp.push(data);
      }
    })
                ...
                <UnBoxItem className="uncompletebox-with-line">
                  <BetweenRow style={{width:"100%"}}>
                    ...
                    <Row>
                      ...
                      <CompleteBtn onClick={() => { _handleComplete(data) }}>완료</CompleteBtn>
                    </Row>
