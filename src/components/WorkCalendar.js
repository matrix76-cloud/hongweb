...
`
const WorkCalendar = ({ containerStyle, callback }) => {
  const { dispatch, user } = useContext(UserContext);
  ...
  const _handleWorkDetailPopupClose = async(data) => {
    ...
    homeworkitemsTmp.map((data) => {
      if (data.complete == false) {
        eventsTmp.push({ title: data.MEMO, start: new Date(data.STARTDATE), end: new Date(data.STARTDATE), color: '#FFF', backgroundcolor: '#FFA95E', opacity: 0.8 })
      } else {
  ...
  return (
    <Container>
      ...
      {workdetailpopup == true && <MobileWorkDetailPopup callback={_handleWorkDetailPopupClose} workitems={workitems} startdate={selectstartorigin} />}
