  ...
  const _handlesend = async (forcedText) => {
    ...
    try {
      ...
      if (preset?.id === "fortune") {
        ...
        // 프로필 없으면: 폼카드 or 텍스트로 1문장만 물어보기
        if (!profile) {
          if (SHOW_FORTUNE_FORM) {
            await CeateAITextMessage({
              CHAT_ID: chatid,
              text: "", // 텍스트 말풍선 없이 폼만
              item: {
                ...location.state.ITEM,
                AI_CARD: { type: "fortune_form", initial: {} },
              },
              read: [user.USERS_ID],
            ...
            const payload = JSON.stringify({ message: userInput, context: {} });
            const resp = await chatSession.sendMessage(
              "필요하면 한 문장으로만 추가 정보를 물어보고, 가능하면 바로 답해. 코드블록 금지.\n" + payload
            );
            const aiText = (await resp.response.text()).trim();
            await CreateMessage({
              CHAT_ID: chatid,
              msg: aiText,
              users_id: "ai-friend-id",
        ...
        const payload = JSON.stringify({
          ...
          context: {
            fortune_profile: {
              ...
              lunar: !!profile.lunar,    // true=음력
              tz: profile.tz || "Asia/Seoul",
            },
          },
        });

        const resp = await chatSession.sendMessage(
          "fortune_profile가 있으면 그 정보를 사용해서 바로 답해. 다시 물어보지 마. 코드블록 금지.\n" + payload
        );
        const aiText = (await resp.response.text()).trim();
      ...
      if (preset?.id === "camp") {
        ...
        // 3-2) 모델로 검색계획/지식판단
        const intentHint = /(준비|챙길|필요|패킹|장비|용품|필요)/.test(userInput) ? "PACKING_LIST" : undefined;
        const payload = JSON.stringify({ message: userInput, context: { intent_hint: intentHint } });
        const r1 = await chatSession.sendMessage("search 또는 answer JSON만 출력. 코드블록 금지.\n" + payload);
        const aiText1 = (await r1.response.text()).trim();
        const plan = parseAIPlan(aiText1);
      ...
      /* ─────────────────────────────
         4) 기본 프리셋 (그냥 물어보기)
         ───────────────────────────── */
      const r = await chatSession.sendMessage(userInput);
      const aiPlain = await r.response.text();
